...
if ($searchTerm !== '') {
    ...
    $stmt = $pdo->prepare("Select Count(*) as total FROM Appointment WHERE PatientID LIKE :search");
    $stmt->execute(['search' => "%$searchTerm%"]);
}   else {
    //if there is no search term available then it will count all of the records
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM Appointment");
}
...
// Just fetching the Appointment records based on current page now. this is with or without the search term
if ($searchTerm !== '') {
    $stmt = $pdo->prepare("SELECT * FROM Appointment WHERE PatientID Like :search LIMIT :limit OFFSET :offset");
    ...
    $stmt = $pdo->prepare("SELECT * FROM Appointment LIMIT :limit OFFSET :offset"); 
...
$appointments = $stmt->fetchALL(PDO::FETCH_ASSOC); //This will fetch all of the Appointment records as a associative array
?>
...
<body>
        ...
        ?>
    <div>
        <h2 class="centered-header">List Of Appointments</h2>
    </div>

    <!-- Search Form -->

    <form class="SearchForm" method="GET" action="">
            <input id="SearchBar" type="text" name="search" placeholder="Search Appointments..." value="<?=htmlspecialchars($_GET['search'] ?? '') ?>">
    ...
    <div>
      ...
      <?php
        ...
        echo "<table>";
        echo "<tr> <th>Appointment ID</th> <th>Room ID</th> <th>Patient ID</th> <th>Staff ID</th><th>Appointment Time</th><th>Appointment Date</th> <th>Extra Notes</th> <th>Actions</th></tr>";
        ...
        foreach ($appointments as $row) {
            ...
            $ExtraNotes = htmlspecialchars($row['ExtraNotes']);
           ...
           echo "<tr> 
                  ...
                  <td>$ExtraNotes</td>
                  <td> 
                   <a href='updateAppointmentPage.php?AppointmentID=$AppointmentID'> Update </a> &nbsp;&nbsp;
                   <a href='deleteAppointment.php?AppointmentID=$AppointmentID' onclick='return confirm(\"Are you sure you would like to delete this record?\");' class='action-link delete'>Delete</a>
                </tr>";
        }
        echo "</table>";
