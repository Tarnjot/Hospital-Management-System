...
if ($searchTerm !== '') {
    ...
    $stmt = $pdo->prepare("Select Count(*) as total FROM ContactUs WHERE FullName LIKE :search OR Email Like :search");
    $stmt->execute(['search' => "%$searchTerm%"]);
}   else {
    //if there is no search term available then it will count all of the records
    $stmt = $pdo->query("SELECT COUNT(*) as total FROM ContactUs");
}
...
// Just fetching the ContactUs records based on current page now. this is with or without the search term
if ($searchTerm !== '') {
    $stmt = $pdo->prepare("SELECT * FROM ContactUs WHERE FullName Like :search OR Email LIKE :search LIMIT :limit OFFSET :offset");
    ...
    $stmt = $pdo->prepare("SELECT * FROM ContactUs LIMIT :limit OFFSET :offset"); 
...
$appointments = $stmt->fetchALL(PDO::FETCH_ASSOC); //This will fetch all of the appointments records as a associative array
...
<body>
        ...
        ?>
    <div>
        <h2 class="centered-header">Contact Us</h2>
    </div>

     <!-- Search Form -->

     <form class="SearchForm" method="GET" action="">
            <input id="SearchBar" type="text" name="search" placeholder="Search ContactUs..." value="<?=htmlspecialchars($_GET['search'] ?? '') ?>">
    ...
    <div>
      <?php
        ...
        echo "<table>";
        echo "<tr> <th>ContactUs</th> <th>Full Name</th> <th>Email</th> <th>Message</th><th>Actions</th></tr>";

        foreach ($appointments as $row) {
            $ContactUs = htmlspecialchars($row['ContactUs']);
           ...
           echo "<tr> 
                  ...
                  <td>$Message</td>
                  <td> 
                   <a href='deleteContactUs.php?ContactUs=$ContactUs' onclick='return confirm(\"Are you sure you would like to delete this record?\");' class='action-link delete'>Delete</a>
                </tr>";
        }
        echo "</table>";
