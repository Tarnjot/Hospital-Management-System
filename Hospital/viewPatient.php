...
if ($searchTerm !== '') {
    ...
    $stmt = $pdo->prepare("Select Count(*) as total FROM Patient WHERE FName LIKE :search OR LName Like :search");
...
// Just fetching the patient records based on current page now. this is with or without the search term
if ($searchTerm !== '') {
    $stmt = $pdo->prepare("SELECT * FROM Patient WHERE FName Like :search OR LName LIKE :search LIMIT :limit OFFSET :offset");
...
$patients = $stmt->fetchALL(PDO::FETCH_ASSOC); //This will fetch all fo the patient records as a associative array
...
<body>
        ...
        ?>
    <div>
        <h2 class="centered-header">List Of Patient</h2>
    </div>

    <!-- Search Form -->

    <form class="SearchForm" method="GET" action="">
            <input id="SearchBar" type="text" name="search" placeholder="Search Patients..." value="<?=htmlspecialchars($_GET['search'] ?? '') ?>">
    ...
    <div>
      <?php
        ...
        echo "<table>";
        echo "<tr> <th>Patient ID</th> <th>First Name</th> <th>Last Name</th> <th>Date Of Birth</th><th>PhoneNo</th><th>Email</th> <th>Gender</th> <th>Address</th><th>Actions</th></tr>";
        ...
        foreach ($patients as $row) {
           ...
           echo "<tr> 
                  ...
                  <td>$Address</td>
                  <td> 
                   <a href='updatePatientPage.php ?PatientID=$PatientID'> Update </a> &nbsp;&nbsp;
                   <a href='deletePatient.php?PatientID=$PatientID' onclick='return confirm(\"Are you sure you would like to delete this record?\");' class='action-link delete'>Delete</a>
                </tr>";
        }
        ...
        //Adding a query string if there is a search term involved
        $queryString = '';
        if (!empty($searchTerm)) {
