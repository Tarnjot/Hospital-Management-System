...
if ($searchTerm !== '') {
    ...
    $stmt = $pdo->prepare("Select Count(*) as total FROM Room WHERE RoomType LIKE :search");
...
// Just fetching the Room records based on current page now. this is with or without the search term
if ($searchTerm !== '') {
    $stmt = $pdo->prepare("SELECT * FROM Room WHERE RoomType Like :search LIMIT :limit OFFSET :offset");
    ...
    $stmt = $pdo->prepare("SELECT * FROM Room LIMIT :limit OFFSET :offset"); 
...
$room = $stmt->fetchALL(PDO::FETCH_ASSOC); //This will fetch all fo the Room records as a associative array
...
<body>
        ...
        ?>
    <div>
        <h2 class="centered-header">List Of Rooms</h2>
    </div>

    <!-- Search Form -->

    <form class="SearchForm" method="GET" action="">
            <input id="SearchBar" type="text" name="search" placeholder="Search Room..." value="<?=htmlspecialchars($_GET['search'] ?? '') ?>">
    ...
    <div>
      <?php
        ...
        echo "<table>";
        echo "<tr> <th>Room ID</th> <th>Room Type</th> <th>Room Capacity</th> </tr>";

        foreach ($room as $row) {
            $RoomID = htmlspecialchars($row['RoomID']);
            $RoomType = htmlspecialchars($row['RoomType']);
            $RoomCapacity = htmlspecialchars($row['RoomCapacity']);
           ...
           echo "<tr> 
                  ...
                  <td>$RoomCapacity</td>
                  <td> 
                   <a href='updateRoomPage.php ?RoomID=$RoomID'> Update </a> &nbsp;&nbsp;
                   <a href='deleteRoom.php?RoomID=$RoomID' onclick='return confirm(\"Are you sure you would like to delete this record?\");' class='action-link delete'>Delete</a>
                </tr>";
        }
        echo "</table>";
